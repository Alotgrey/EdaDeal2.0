...
class SberCategoriesParser:
    ...
    def __collect_raw_json_data_for_get_retailers(
        self,
    ) -> dict:
        """
        ...
        with open(constants.RETAILERS_LIST_PATH, "w") as file:
            json.dump(data, file)
        logger.info(
            f"Собрали все данные и сохранили их в {constants.RETAILERS_LIST_PATH}"
        )
        driver.close()
        return data
    ...
    def get_retailers(self) -> dict:
        ...
        if os.path.exists(constants.RETAILERS_LIST_PATH):
            if (
                ...
                - datetime.datetime.fromtimestamp(
                    os.path.getctime(constants.RETAILERS_LIST_PATH)
                )
            ).days <= 15:
                logger.warning(
                    f"Прошло 15 дней с момента создания файла {constants.RETAILERS_LIST_PATH}. Проверьте нужно ли ему обновление!"
                )
            logger.info(
                f"Нашли файл {constants.RETAILERS_LIST_PATH}. Собираем данные от туда!"
            )
        ...
        retailers_data = data["props"]["pageProps"]["storefrontProps"][
            "reduxStoreState"
        ]["landing"]["retailers"]["entities"]
        retailers = {
            retailer_info["name"]: retailer_info["slug"]
            for retailer_id, retailer_info in retailers_data.items()
        }
